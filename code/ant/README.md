...
Same SF-MAML pipeline as HalfCheetah, run on Ant-v4. Steps 1 through 7 are
identical scripts, just pointed at Ant. Step 8 (the downstream eval) uses a
different script than HalfCheetah did - explained below.
...
ant/
  cleanrl/                 (keep this name, imports depend on it)
    ...
    cleanrl/                pipeline scripts
      ...
      ddpgtesting19.py       <- downstream eval, note the name (see step 8)
...
Run everything from the `ant` folder (one level above `cleanrl/`), using
`python -m`, same as HalfCheetah. Checkpoints and data land under `runs/`,
each script prints where it saved to.

### 1. Train DIAYN

```
python -m cleanrl.cleanrl.dqn2_cont --env-id Ant-v4
```
...
Same as before - go through the per-skill videos and note down 6 skills
that look misaligned with normal forward locomotion.
...
Same as HalfCheetah - open `cleanrl/ddpg_continuous_action.py`, find the
block marked "comment theses lines when not required", uncomment those 5
lines, run:

```
python -m cleanrl.cleanrl.ddpg_continuous_action --env-id Ant-v4
```
...
### 8. Downstream adaptation - `ddpgtesting19.py`, not `ddpgtest.py`
...
For HalfCheetah, the downstream script only put the successor features into
the critic. For Ant, `ddpgtesting19.py` also copies the successor feature
network's first layer into the actor and freezes it, so the actor starts
from the meta-learned representation too, not just the critic. That's a real
difference between the two runs, not a bug - keeping it as is.

`--no-pretrained` runs it as the Base comparison (no successor feature
injection anywhere). `--disc-path` is in the args but unused, same as
HalfCheetah.
