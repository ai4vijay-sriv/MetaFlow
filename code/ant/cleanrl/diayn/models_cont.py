...
class SFNetworkbig(nn.Module):
    ...
    def first_layer_activation(self, state, action):
        """Return the first-layer activation h^1 for progressive lateral adapter (size 256)."""
        x = torch.cat([state, action], dim=-1)
...
class SFNetwork(nn.Module):
    ...
    def first_layer_activation(self, state, action):
        """Return the first-layer activation h^1 for progressive lateral adapter."""
        x = torch.cat([state, action], dim=-1)
        return F.relu(self.l1(x))
    
    def second_layer_activation(self, state, action):
        """Return the second-layer activation h^2 for progressive lateral adapter."""
        x = torch.cat([state, action], dim=-1)
        x = F.relu(self.l1(x))
        return F.relu(self.l2(x))

class SFNetworksmall(nn.Module):
    def __init__(self, state_dim, action_dim, sf_dim=32):
        super(SFNetworksmall, self).__init__()
        self.input_dim = state_dim + action_dim
        self.sf_dim = sf_dim

        self.l1 = nn.Linear(self.input_dim, 64)
        self.l2 = nn.Linear(64, 84)
        self.l3 = nn.Linear(84, sf_dim)
