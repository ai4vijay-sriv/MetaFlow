...
class Args:
    ...
    data_path: str = "runs/data/<your_data3_run>/maml_training_data.pkl"
    """from data3.py"""
    disc_path: str = "runs/checkpoints/qtargetmaml/<your_q_online_run>/latest.pth"
    """from q_online.py"""
    qnet_path: str = "runs/checkpoints/qtargetmaml/<your_q_online_run>/latest.pth"
    sf_dim: int = 32
    n_skills_total: int = 25
    n_skills_selected: int = 6
    n_skills_epoch: int = 4
    n_actions: int = 4  # maze-random-10x10-plus-v0 has 4 discrete actions
    hidden_dim: int = 120
    inner_lr: float = 1e-3
...
def train():
    ...
    # number of inner loop updates 
    # skill indices picked in step 2, keep this in sync with q_online.py and data3.py
