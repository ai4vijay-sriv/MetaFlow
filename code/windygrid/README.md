...
Same DQN-based pipeline as the other discrete envs, run on WindyGridWorld-v0
(30x30 grid, the config the paper reports).

## Extra setup: gym-windy-gridworlds

Unlike the other environments, this one isn't a standard Gymnasium/PyPI
package. Install it separately before doing anything else:

```
pip install git+https://github.com/ibrahim-elshar/gym-windy-gridworlds
```

The grid size (30x30) is just the environment class's own constructor
default, not something passed in through these scripts, so nothing extra
needs configuring for that.
...
Run everything from the `windygrid` folder (one level above `cleanrl/`),
...
```
python -m cleanrl.cleanrl.dqn2 --env-id WindyGridWorld-v0
```
...
normal shortest-path policy through the wind.
...
```
python -m cleanrl.cleanrl.dqn --env-id WindyGridWorld-v0
```
